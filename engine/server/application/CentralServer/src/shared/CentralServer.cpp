...
void CentralServer::launchStartingProcesses() const
{
	...
	// Launch CustomerServiceServer
	std::string csOptions = "-s CustomerServiceServer centralServerAddress=" + getAddress(getCustomerService());
	csOptions += std::string(" clusterName=") + ConfigCentralServer::getClusterName();
	TaskSpawnProcess csProcess("any", "CustomerServiceServer", csOptions);
	m_taskManager->send(csProcess, true);
}
